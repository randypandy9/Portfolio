...
public class BlackjackTable implements Serializable {
    ...
    public static void advancedGame() throws FileNotFoundException,
            IOException {
        //carry on for set number of runs
        boolean XXX = true;
        while (XXX = true) {
            ...
            while (res > 0) {
                ...
                System.out.println(dealer.getDealersHand() + "SCORE: "
                        + dealers + "\n");
                //comparisons are made between players and 
                //dealer to see who wins
                dealer.settleBets();
                //reset hands ready for new game
                for (int i = 0; i < players.size(); i++) {
                    players.get(i).newHand();
